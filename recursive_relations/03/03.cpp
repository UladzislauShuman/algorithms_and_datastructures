...
{
    for (int i = 1; i <= n; ++i) factorial[i] = (factorial[i - 1] * i) % MOD_NUM; 
}
...
std::vector<long long> createFactorialInverseFromFactorial(std::vector<long long>& factorial, int n)
{
    ...
    for (int i = n - 1; i >= 1; --i) factorial_inverse[i] = (factorial_inverse[i + 1] * (i + 1)) % MOD_NUM; // 1/(n * n-1 * ...) * n = 1/(n-1 * n-2 *...) 
    return factorial_inverse;
}

long long getInversForMod(long long a) {
    long long b = 1; // a * b == 1 mod MOD_NUM
    long long pow = MOD_NUM - 2; // из т Эйлера 
    while (pow > 0) { // чтобы посчитать a^(m-2) -- бинарное возведение в степень
        if (pow & 1)  // если 1 -- нечётное, 0 -- чётное
            b = (b * a) % MOD_NUM;
        a = (a * a) % MOD_NUM;
        pow >>= 1;
    }
...
int main() 
{
    ...
    cout << (((factorial[n] * factorial_inverse[k]) % MOD_NUM) * factorial_inverse[n - k]) % MOD_NUM;
    return 0;
}
